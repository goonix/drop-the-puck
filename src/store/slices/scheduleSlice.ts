...
export const loadSchedule = createAsyncThunk('schedule/loadSchedule', async (date: string) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loadSchedule.fulfilled, (state, action) => {
        ...
        state.hasLiveGames = games.some((g) => g.gameState === 'LIVE' || g.gameState === 'CRIT');
      })
...
export const reducer = scheduleSlice.reducer;
