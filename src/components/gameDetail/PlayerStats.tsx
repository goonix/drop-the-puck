...
export default function PlayerStats({ playerStats, awayAbbrev, homeAbbrev }: Props) {
  return (
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-gray-700/20 last:border-0">
      ...
      <td className="py-1.5 px-1">
        ...
        <span className="ml-1 text-gray-400 dark:text-gray-600">{isForward ? p.position : 'D'}</span>
      </td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{p.goals}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{p.assists}</td>
      <td className="py-1.5 px-1.5 text-center font-semibold text-gray-900 dark:text-white tabular-nums">{p.points}</td>
      <td className={`py-1.5 px-1.5 text-center tabular-nums ${p.plusMinus > 0 ? 'text-green-600 dark:text-green-400' : p.plusMinus < 0 ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>
        {p.plusMinus > 0 ? `+${p.plusMinus}` : p.plusMinus}
      </td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{p.sog}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{p.hits}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-500 dark:text-gray-400 tabular-nums">{p.toi}</td>
    </tr>
  )
}
...
function GoalieRow({ goalie: g }: { goalie: GoalieStat }) {
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-gray-700/20 last:border-0">
      ...
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{g.shotsAgainst}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{g.saves}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{g.goalsAgainst}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-700 dark:text-gray-300 tabular-nums">{savePct}</td>
      <td className="py-1.5 px-1.5 text-center text-gray-500 dark:text-gray-400 tabular-nums">{g.toi}</td>
    </tr>
  )
}
