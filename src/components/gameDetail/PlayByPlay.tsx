...
export default function PlayByPlay({ plays, players }: Props) {
  const reversed = [...plays].reverse()

  return (
    <div className="px-4 py-2">
      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Play by Play</h3>
...
function PlayRow({ play, players }: { play: Play; players: Record<number, PlayerInfo> }) {
  ...
  const primaryPlayer = d.scoringPlayerId || d.winningPlayerId || d.shootingPlayerId ||
    d.hittingPlayerId || d.committedByPlayerId || d.playerId
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 border-b border-gray-100 dark:border-gray-700/20 last:border-0 text-xs">
      <span className="text-gray-400 dark:text-gray-600 w-8 text-right tabular-nums">{play.timeInPeriod}</span>
      <span className="text-gray-400 dark:text-gray-600 w-6">{periodLabel(play.periodType, play.period)}</span>
