...
export default function BracketTreeCard({ series }: Props) {
...
function TeamRow({
  ...
  return (
    <div className={`flex items-center gap-1.5 ${isLoser ? 'opacity-40' : ''}`}>
      <span className="text-xs text-gray-400 dark:text-gray-500 w-5 shrink-0 tabular-nums">{seed}</span>
      {team ? (
      ...
      <span className={`flex-1 text-xs truncate ${isWinner ? 'font-bold text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>
        {team?.abbrev ?? 'TBD'}
      </span>
      {inProgress && (
        <span className={`text-xs font-bold tabular-nums ${isWinner ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
          {wins}
        </span>
      )}
    </div>
